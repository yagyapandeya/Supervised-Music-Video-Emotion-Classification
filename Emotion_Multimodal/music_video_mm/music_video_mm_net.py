...
def video_net(video_shape_slow, video_shape_fast, audio_shape):
    ...
    final_out = concatenate([gap_3D_slow, gap_3D_fast, gap_2D_slow])
    final_out = Dense(6, activation='softmax', name='AV_OUT')(final_out)
    
    model = Model(inputs=[video_input_slow, video_input_fast, audio_input], outputs=[final_out])
    model.summary()
    
    model.load_weights('./music_video_mm_best.h5', by_name = True)
