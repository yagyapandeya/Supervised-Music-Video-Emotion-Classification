...
from music_video_mm_net import video_net
...
video3D_fast, video3D_slow, melphase, target = loadeddata["video3D_fast"], loadeddata["video3D_slow"], loadeddata["melphase"], loadeddata["target"]

#video3D_C3D = normalize(video3D_C3D)
video3D_fast = normalize(video3D_fast)
video3D_slow = normalize(video3D_slow)
#video3D_face = normalize(video3D_face)

melphase = normalize(melphase)
#melphase_fast = normalize(melphase_fast)
#melphase_slow = normalize(melphase_slow)

img_rows, img_cols, frames, frames_slow, vid_channel  = 128, 128, 64, 8, 3
video_input_size = (img_rows, img_cols, frames, vid_channel)
audio_input_size = (128, 1292, 2)

video_input_fast = (img_rows, img_cols, frames, vid_channel)
video_input_slow = (img_rows, img_cols, frames_slow, vid_channel)
video_input_face = (img_rows, img_cols, frames, vid_channel)

audio_input_fast = (128, 1876, 2)
audio_input_slow = (128, 469, 2)

network = video_net(video_input_fast, video_input_slow, audio_input_size)

loss, acc = network.evaluate(x=[video3D_fast, video3D_slow, melphase], y=target, batch_size=5, verbose=0)
print('Test loss:', loss)
print('Test accuracy:', acc)

# make a prediction
#y_pred_keras = network.predict([video_input_fast, video_input_slow, audio_input_fast, audio_input_slow]) 
predictions = network.predict(x=[video3D_fast, video3D_slow, melphase],batch_size=5, verbose=0)    #x=X_test
...
#predictions = network.predict(x=[video3D_fast, melphase],batch_size=10, verbose=0)    #x=X_test
