...
from music_net import audio_net
...
#melphase, target = loadeddata["melphasegram"],loadeddata["target"]
melphase, target = loadeddata["melphase"], loadeddata["target"]
melphase = normalize(melphase)
audio_input_size = (128, 1292, 2)


network = audio_net(audio_input_size)

loss, acc = network.evaluate(x=[melphase], y=target, batch_size=10, verbose=0)
print('Test loss:', loss)
print('Test accuracy:', acc)

# make a prediction
predictions = network.predict(x=[melphase],batch_size=10, verbose=0)    #x=X_test
